...
BuildPaymentQueue(leadName) {
    global agentName, agentEmail, holidays
    leadName := ExtractFirstName(ProperCase(leadName))
    mondays  := BuildNextThreeMondays(holidays)

    sig := "`n`n" . agentName
        . "`nAgente de Seguros – Allstate"
        . "`nDirect Line: (561) 220-7073"
        . "`nOffice Line: (754) 236-8009"
        . "`n" . agentEmail
        . "`nReply STOP to unsubscribe"

    msg1 := "Hola " . leadName . ",`n`n"
        . "Su póliza de Allstate está activa y protegiéndole a usted y a su familia en este momento. Esa protección es real y tiene valor.`n`n"
        . "Para mantenerla vigente, hay un pago pendiente que necesita su atención. Queremos asegurarnos de que usted no pierda la cobertura que ya tiene.`n`n"
        . "Responda *Pagar* y le enviamos el enlace de pago directamente aquí."
        . sig

    msg2 := "Hola " . leadName . ",`n`n"
        . "La mayoría de nuestros clientes que han tenido un pago pendiente lo resolvieron en menos de 5 minutos. Usted también puede hacerlo.`n`n"
        . "Cuando eligió protegerse con Allstate, tomó una decisión importante para su familia. No permita que un aviso de cancelación cambie eso.`n`n"
        . "Allstate le notifica oficialmente que su póliza está en riesgo de cancelación pendiente. Actúe hoy para mantener su protección.`n`n"
        . "Responda *Revisar* y le ayudamos ahora mismo."
        . sig

    msg3 := "Hola " . leadName . ",`n`n"
        . "⚠️ AVISO FINAL — Su cobertura de seguro vence HOY si no se realiza el pago.`n`n"
        . "Si su póliza se cancela: quedará sin protección, sin cobertura en caso de accidente, y podría enfrentar multas por conducir sin seguro.`n`n"
        . "No pierda lo que ya tiene protegido. Haga el pago ahora antes de que sea demasiado tarde.`n`n"
        . "Responda *Pagar* ahora."
        . sig

    return [
        Map("text", msg1, "date", mondays[1], "time", "09:00:00 AM"),
        Map("text", msg2, "date", mondays[2], "time", "10:00:00 AM"),
        Map("text", msg3, "date", mondays[3], "time", "09:30:00 AM")
    ]
}
