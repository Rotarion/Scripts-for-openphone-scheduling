...
MAX_LEAD_NAME_LENGTH := 30
MAX_BATCH_AUTOMATION_VEHICLES := 2
VEHICLE_PRICE_MAP := Map(0, "$98", 1, "$117", 2, "$176", 3, "$284", 4, "$397", 5, "$397")
...
; Extract first name from "Juan Rivera" -> "Juan"
...
; Focus the OpenPhone Slate composer via DevTools console
FocusSlateComposer() {
    Send "^+j"
    Sleep 600
...
; Parse the smashed CRM "PERSONAL LEAD" format for prospect form filling
...
; Extract address, city, state, zip from the smashed location block
; e.g. "5867 Linden Rd , FLHaines CityFlorida33844"
; e.g. "4510 NW 79th Ave #1D Doral, FL 33166Doral,Florida33166"
ParseBatchLocationBlock(locBlock, fields) {
    ; Find the LAST "FullStateName" + "5-digit zip" in the block
    ...
    ; Everything before the full state name = address + city
    beforeState := Trim(SubStr(locBlock, 1, lastPos - 1))
    beforeState := RegExReplace(beforeState, "[,\s]+$", "")

    ; Split on the state abbreviation: "Address , StateAbbr [zip] City"
    ...
    if RegExMatch(beforeState, "i)^(.*?)\s*,\s*(" . stateAbbrs . ")\s*(?:\d{5}(?:-\d{4})?)?\s*(.*)$", &mc) {
        ...
        if (cityPart != "") {
            ...
            ; If city name is duplicated at end of address, strip it
        ...
        SetAddressFields(fields, beforeState)
    }
}
...
; 3-tier initial quote time rule
...
ScheduleMessage(msgText, dateMDY, time12) {
    if !PasteMessageBlockToComposer(msgText)
        return false
    OpenScheduleDialog()
    Sleep 300
    return ConfirmScheduleDateTime(dateMDY . " " . time12)
}
...
ScheduleMessageTyped(msgText, dateMDY, time12) {
    ...
    ConfirmScheduleDateTimeTyped(dateMDY, time12)
...
BuildMessage(leadName, carCount, vehicles := "") {
    global agentName, agentEmail, VEHICLE_PRICE_MAP
    ...
    price  := VEHICLE_PRICE_MAP.Has(carCount) ? VEHICLE_PRICE_MAP[carCount] : "$127"
...
; Splits continuous CRM string on PERSONAL LEAD boundaries
...
; Extracts name from "PERSONAL LEAD - Name   NEW webform..."
...
; Extracts phone from smashed text like "32905(786) 246-9590millycolon72"
...
; Extracts vehicles from after Male/Female marker, splits on year boundaries
...
; Builds a batch lead record using dedicated batch extractors
...
; Clipboard-paste builder scheduler (avoids multi-bubble from SendText newlines)
ScheduleBuilderForLead(lead, offset) {
    ...
    if !PasteMessageBlockToComposer(msg)
        return false
    Sleep 400
    ...
    ConfirmScheduleDateTimeTyped(dt["date"], dt["time"])
    Sleep SLOW_AFTER_DT_PASTE
...
RunBatchLeadFlow(lead) {
    ...
    validationStatus := ValidateBatchLeadRecord(lead)
    ...
    Sleep 300
    if !ApplyCurrentTagValue(lead["TAG_VALUE"])
        return "FAILED - Could not paste holder into tag field"
    return "OK"
}
...
ValidateLeadNameInput(leadName) {
    global MAX_LEAD_NAME_LENGTH
    return !(leadName = "" || StrLen(leadName) > MAX_LEAD_NAME_LENGTH)
}

ReadClipboardLeadNameOrWarn(emptyMessage := "Clipboard empty. Copy the lead's name first."
    , invalidMessage := "") {
    global MAX_LEAD_NAME_LENGTH
    if !ClipWait(1) {
        MsgBox(emptyMessage)
    ...
    if (invalidMessage = "")
        invalidMessage := "Please copy just the lead's name (<= " MAX_LEAD_NAME_LENGTH " chars)."

    if !ValidateLeadNameInput(leadName) {
...
PromptForVehicleCount() {
    ...
    if !(choice ~= "^(0|1|2|3|4|5)$") {
        MsgBox("OpciÃ³n invÃ¡lida. Usa 0, 1, 2, 3, 4 o 5.")
        return ""
    }
    return Integer(choice)
}

ValidateBatchLeadRecord(lead) {
    global MAX_BATCH_AUTOMATION_VEHICLES

    if (lead["PHONE"] = "" || lead["FULL_NAME"] = "")
        return "SKIPPED - Missing phone or name"

    if (lead["VEHICLE_COUNT"] > MAX_BATCH_AUTOMATION_VEHICLES)
...
PasteMessageBlockToComposer(msgText) {
    if !SetClip(msgText) {
...
ConfirmScheduleDateTime(dateTimeText) {
    if !SetClip(dateTimeText) {
...
ApplyCurrentTagValue(tagValue) {
    global BATCH_AFTER_ENTER, BATCH_BEFORE_TAG_PASTE, BATCH_AFTER_TAG_PASTE
...
RunInitialQuoteBuilderHotkey() {
    leadName := ReadClipboardLeadNameOrWarn()
    if (leadName = "")
        return

    carCount := PromptForVehicleCount()
    if (carCount = "")
        return
    ...
    SetClip(ProperCase(leadName))
    ClipWait(1)
}

RunFollowupQueueHotkey(useStablePath) {
    global MAX_LEAD_NAME_LENGTH
    leadName := ReadClipboardLeadNameOrWarn(
        "Clipboard empty. Copy the lead's name first.",
        "Please copy just the lead's name (<= " MAX_LEAD_NAME_LENGTH " chars) and try again."
    )
    if (leadName = "")
        return

    offset := NextRotationOffset()
    msgs := BuildFollowupQueue(leadName, offset)
...
RunTagOnlyHotkey() {
    ...
    Sleep 300
    if !ApplyCurrentTagValue(tagSymbol)
        MsgBox("Failed to paste tag value.")
}
...
^!8:: {
    global iniFile, holidays, configDays, MAX_LEAD_NAME_LENGTH
...
^!g:: {
    RunTagOnlyHotkey()
    return
    global tagSymbol, BATCH_AFTER_ENTER, BATCH_BEFORE_TAG_PASTE, BATCH_AFTER_TAG_PASTE
