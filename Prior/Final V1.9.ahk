...
BuildFollowupQueue(leadName, offset) {
    ...
    leadName := FirstName(ProperCase(leadName))
...
ExtractBatchName(raw) {
    if RegExMatch(raw, "i)PERSONAL\s+LEAD\s*-\s*(.+?)\s{2,}", &m)
        return Trim(m[1])
    if RegExMatch(raw, "i)PERSONAL\s+LEAD\s*-\s*(.+?)(?:\s+NEW\b)", &m)
        return Trim(m[1])
    return ""
}
...
RunBatchLeadFlow(lead) {
    ...
    ; Tab out of tag area to re-enable Alt+N for next lead
    SendTabs(2)
    Sleep 200
