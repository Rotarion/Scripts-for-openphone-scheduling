...
NormalizeAddressMap(fields) {
    if (fields["ADDRESS_1"] != "")
        SetAddressFields(fields, fields["ADDRESS_1"])

    fields["CITY"] := ProperCasePhrase(fields["CITY"])
    fields["STATE"] := NormalizeState(fields["STATE"])
    fields["ZIP"] := NormalizeZip(fields["ZIP"])
    fields["PHONE"] := NormalizePhone(fields["PHONE"])
}
...
NormalizeDOB(dob) {
    ...
    ; Lowercase working copy for cleanup
    work := StrLower(text)

    ; Remove common noise
    work := RegExReplace(work, "\bconfirm\b", "")
    work := RegExReplace(work, "\bage\s*\d{1,3}\b", "")
    work := RegExReplace(work, "\b\d{1,3}\s*años\b", "")
    work := RegExReplace(work, "\b\d{1,3}\s*anos\b", "")
    work := RegExReplace(work, "\bnacida?\s+en\b", "")
    work := RegExReplace(work, "\bnacido\s+en\b", "")
    work := RegExReplace(work, "\bedad\s*\d{1,3}\b", "")
    work := RegExReplace(work, "[\(\)]", "")
    work := RegExReplace(work, "\s*/\s*\d{1,3}\s*(?:años|anos)?", "")
    work := RegExReplace(work, "^\d{1,3}\s*,\s*", "") ; removes leading age like "59, April 26 1966"
    work := RegExReplace(work, ",", " ")
    work := RegExReplace(work, "\s+", " ")
    work := Trim(work)

    ; Normalize Spanish month names to English abbreviations or month numbers
    work := NormalizeMonthWords(work)

    ; 1) dd-mon-yy or dd-mon-yyyy
