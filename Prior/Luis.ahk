...
LeadFullName(fields) {
    return Trim(fields["FIRST_NAME"] . " " . fields["LAST_NAME"])
}

IsBatchLeadLine(line) {
    line := Trim(line)
    if (line = "")
        return false
    return RegExMatch(line, "i)\bPERSONAL LEAD\b")
}
...
ParseBatchLeadRows(raw) {
    ...
    for _, line in StrSplit(clean, "`n") {
        line := Trim(line)
        if IsBatchLeadLine(line)
            rows.Push(line)
    }
    return rows
}

IsVehicleToken(token) {
    token := Trim(token)
    if (token = "")
        return false
    ; starts with a likely model year and has text after it
    return RegExMatch(token, "i)^(19|20)\d{2}\s+[A-Za-z].+")
}

ExtractVehicleList(rawLead) {
    vehicles := []
    tokens := TokenizeLead(rawLead)

    for _, token in tokens {
        token := Trim(token)
        if !IsVehicleToken(token)
            continue
        vehicles.Push(ProperCasePhrase(token))
    }
    return vehicles
}

GetBatchBuilderTime(offset) {
    target := TimeWithOffset(12, 0, 0, offset)

    nowHHMMSS := FormatTime(A_Now, "HHmmss")

    today := FormatTime(A_Now, "yyyyMMdd")
    targetStamp := today . " " . target
    targetHHMMSS := FormatTime(targetStamp, "HHmmss")

    if (nowHHMMSS > targetHHMMSS)
        return "05:58:00 PM"
...
BuildBatchLeadRecord(rawLead) {
    fields := NormalizeProspectInput(rawLead)
...
ScheduleBuilderForLead(lead, offset) {
    sameDay := FormatTime(A_Now, "MM/dd/yyyy")
    sendTime := GetBatchBuilderTime(offset)
    msg := BuildMessage(lead["FULL_NAME"], lead["VEHICLE_COUNT"])
    return ScheduleMessageTyped(msg, sameDay, sendTime)
}
