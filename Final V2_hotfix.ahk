...
RunDevToolsJSGetResult(jsExpr, waitSeconds := 2) {
    ...
    Send "^+j"
    Sleep 700
    SendText "copy(String(" . jsExpr . "))"
    Sleep 120
    Send "{Enter}"

    ok := ClipWait(waitSeconds)
    result := ok ? Trim(A_Clipboard) : ""
...
TryClickQuoResultByText(targetText) {
    targetText := Trim(targetText)
    if (targetText = "")
        return ""

    escaped := JSEscapeSingleQuoted(targetText)

    js :=
    (
(() => {
    const norm = s => (s || '').replace(/\s+/g, ' ').trim().toLowerCase();
    const target = norm('__TARGET__');
    const targetNoPlus = target.replace(/^\+\s*/, '');
    const docs = [];
    const seen = new Set();
    ...
    const matches = [];
    for (const d of docs) {
        const nodes = d.querySelectorAll('button, [role="button"], div, span');
        for (const node of nodes) {
            const text = norm(node.innerText || node.textContent || '');
            if (!text)
                continue;
            const textNoPlus = text.replace(/^\+\s*/, '');
            if (text === target || textNoPlus === targetNoPlus)
                matches.push(node);
        }
    }
    ...
    let node = matches.find(n => !!(n.offsetParent || n.closest('button')?.offsetParent)) || matches[0];
    let clickable = node.closest('button') || node;

    try { clickable.scrollIntoView({block:'center', inline:'nearest'}); } catch (e) {}

    ['mouseover','mousedown','mouseup','click'].forEach(t => {
        clickable.dispatchEvent(new MouseEvent(t, {
            bubbles: true,
            cancelable: true,
            view: clickable.ownerDocument.defaultView
        }));
    });
    ...
    js := StrReplace(js, "__TARGET__", escaped)
    return RunDevToolsJSGetResult(js)
}

TryClickQuoTagPlusOption() {
    js :=
    (
(() => {
    const docs = [];
    ...
    for (const d of docs) {
        const preferred = [
            ...d.querySelectorAll('.bevq3b0, .zio9s40, .zio9s42'),
            ...d.querySelectorAll('button, [role="button"], div, span')
        ];
        ...
        for (const node of preferred) {
            ...
            const clickable = node.closest('button') || node;
...
PromptBatchManualReview(lead, reason) {
    leadName := lead.Has("FULL_NAME") ? lead["FULL_NAME"] : ""
    leadPhone := lead.Has("PHONE") ? lead["PHONE"] : ""

    msg := "Manual review required.`n`n"
        . "Lead: " leadName "`n"
        . "Phone: " leadPhone "`n"
        . "Reason: " reason "`n`n"
        . "Click OK, then click back into Quo to continue with the next lead.`n"
        . "Click Cancel to stop the batch."

    result := MsgBox(msg, "Batch Manual Review", "OKCancel Icon!")

    if (result = "OK") {
        ToolTip("Manual review: click back into Quo to continue")
        KeyWait "LButton"
        KeyWait "LButton", "D"
        Sleep 150
        ToolTip()
        return "SKIPPED - Manual review - " reason
    }

    return "STOPPED - User canceled during manual review - " reason
}
...
RunBatchLeadFlow(lead) {
    ...
    if (lead["PHONE"] = "" || lead["FULL_NAME"] = "")
        return PromptBatchManualReview(lead, "Missing parsed phone or name")
    ...
    if !PasteValue(lead["PHONE"])
        return PromptBatchManualReview(lead, "Could not paste phone")
    ...
    if !PasteValue(lead["HOLDER_NAME"])
        return PromptBatchManualReview(lead, "Could not paste name lookup value")
    Sleep BATCH_AFTER_NAME_PICK

    namePickResult := TryClickQuoResultByText(lead["HOLDER_NAME"])
    if (namePickResult != "OK")
        namePickResult := TryClickQuoResultByText(lead["FULL_NAME"])

    if (namePickResult != "OK")
        return PromptBatchManualReview(lead, "Name result not found in Quo")
    ...
    if !PasteValue(lead["TAG_VALUE"])
        return PromptBatchManualReview(lead, "Could not paste tag lookup value")
    Sleep BATCH_AFTER_TAG_PASTE

    tagPickResult := TryClickQuoTagPlusOption()
    if (tagPickResult != "OK")
        return PromptBatchManualReview(lead, "Tag result '+' not found in Quo")

    Sleep BATCH_AFTER_TAG_PICK
    SendTabs(2)
    Sleep 200
    return "OK"
}

; Legacy fallback preserved below for emergency rollback.
RunBatchLeadFlowLegacy(lead) {
    global BATCH_AFTER_ALTN, BATCH_AFTER_PHONE, BATCH_AFTER_TAB
    global BATCH_AFTER_SCHEDULE, BATCH_AFTER_ENTER, BATCH_AFTER_NAME_PICK, BATCH_AFTER_TAG_PICK
    global BATCH_BEFORE_TAG_PASTE, BATCH_AFTER_TAG_PASTE
